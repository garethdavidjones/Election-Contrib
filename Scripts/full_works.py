...
def formKeyValues(x):
    ...
    elif contributor_cfscore <= -0.3:
        contributor_cfscore = -1
    elif contributor_cfscore <= 0.3:
        contributor_cfscore = 0
    elif contributor_cfscore <= 1:
        contributor_cfscore = 1
    elif contributor_cfscore > 1:
        contributor_cfscore = 2
    else:
...
def similarity_algo(test, train, k):
    ...
    top_sim = sim_pairs.filter(lambda x: x[1][1] >= k).collect() #Wow, I don't think this will work
...
if __name__ == '__main__':
    ...
    test_file = "gs://cs123data/Data/contribDB_2012.csv"
