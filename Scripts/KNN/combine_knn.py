...
def create_vectors(line):
    ...
    if v_candidate_cfscore != v_contr_cfscore:
        ideoDifference = abs(v_candidate_cfscore) - abs(v_contr_cfscore) #this is not correct. Need to run abs(cand_cf - contr_cf)
        if ideoDifference == 1:
            v_id_diff = 1
    ...
    if party == "100":
        ...
        if v_candidate_cfscore < 0:
            label = 1 #interesting idea. Not sure if wise
        if v_candidate_cfscore >= 0:
            label = 2
